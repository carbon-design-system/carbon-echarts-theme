  ...
  return (
    <div>
      ...
      <div className="content-page">
        ...
        <h2>Trigger types</h2>
        <pre>
          <code>{`// 'axis' — shows all series values at the hovered x position (default for line/bar)
tooltip: { trigger: 'axis' }

// 'item' — shows the value for the specific hovered data point (default for pie/scatter)
tooltip: { trigger: 'item' }`}</code>
        </pre>
  ...
  formatter: (params) => {
    return params.map(p => \`\${p.seriesName}: \${p.value.toLocaleString()}\`).join('<br/>')
        ...
        <h2>Accessibility</h2>
        <p>
          Tooltips are not accessible to keyboard-only or screen-reader users by default. Ensure all
          data shown in tooltips is also available in an accessible data table or the chart has an{' '}
          <code>aria-label</code> describing the key insight.
        </p>
      </div>
    </div>
  )
