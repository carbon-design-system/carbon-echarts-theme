  ...
  return (
    <div>
      ...
      <div className="content-page">
        <p>
          Axes provide the scale and reference frame for chart data. Carbon design guidance for
          axes applies regardless of the charting framework — including ECharts with this theme.
        </p>
        ...
        <pre><code>{`// Categorical x-axis (default for bar charts)
xAxis: { type: 'category', data: categories }
yAxis: { type: 'value' }`}</code></pre>
...
series: [
  ...
  { name: 'Conversion', type: 'line', yAxisIndex: 1, data: [...] },
        ...
        <pre><code>{`yAxis: { type: 'log', logBase: 10 }`}</code></pre>

        <h2>Time axis</h2>
        <pre><code>{`xAxis: { type: 'time' }
// Data points use ISO date strings or timestamps as x values`}</code></pre>
      </div>
    </div>
  )
}
